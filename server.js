...
app.get('/api/rooms', authMiddleware, async (req, res) => {
    try {
        const rooms = await Room.find({ isPrivate: false }).populate('owner', 'username').select('-password');
        res.json(rooms);
    } catch (error) {
...
app.post('/api/rooms/:id/join', authMiddleware, async (req, res) => {
    try {
        ...
        if (room.isPrivate) {
            ...
            if (password !== room.password) return res.status(403).json({ error: 'Geçersiz oda şifresi' });
        }
...
app.get('/api/rooms/:id/messages', authMiddleware, async (req, res) => {
    try {
        const messages = await Message.find({ room: req.params.id }).populate('sender', 'username').sort({ timestamp: -1 }).limit(50);
...
app.put('/api/messages/:id', authMiddleware, async (req, res) => {
    try {
        ...
        if (message.sender.toString() !== req.user._id.toString()) return res.status(403).json({ error: 'Bu mesajı düzenleyemezsin' });
...
app.delete('/api/messages/:id', authMiddleware, async (req, res) => {
    try {
        const message = await Message.findById(req.params.id);
        if (!message) return res.status(404).json({ error: 'Mesaj bulunamadı' });
        if (message.sender.toString() !== req.user._id.toString()) return res.status(403).json({ error: 'Bu mesajı silemezsin' });
