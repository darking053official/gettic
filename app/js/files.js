// ╔══════════════════════════════════════════════════════════════════╗
// ║           GETTIC FILES.JS - WASM SIKIŞTIRMA + SVG              ║
// ╚══════════════════════════════════════════════════════════════════╝
...
const fileState = {
  ...
  currentPreview: null,
  compressThreshold: 100000 // 100KB üstünü sıkıştır
};
...
function handleFile(file) {
  ...
  reader.onload = (e) => {
    ...
    // 📦 WASM Sıkıştırma (büyük dosyalar için)
    if (file.size > fileState.compressThreshold && typeof GetticCompress !== 'undefined') {
      const compressed_data = GetticCompress.compress(data);
      if (compressed_data && compressed_data.length < data.length) {
        data = compressed_data;
        compressed = true;
        console.log('📦 Dosya sıkıştırıldı: ' + formatFileSize(file.size) + ' → ' + formatFileSize(data.length));
      }
    }
    
    uploadFile(file.name, file.type, file.size, data, compressed);
  };
  reader.readAsDataURL(file);
}

function uploadFile(name, type, size, data, compressed = false) {
  const file = { 
    id: genId(), name, type, size, data, 
  ...
  const content = `${fileIcon(icon, 16)} **${escapeHtml(name)}**${compressed ? ' 📦' : ''}`;
  ...
  const msg = { 
    ...
    file: { name, type, size, data, category, compressed } 
  };
  ...
  toast(`${fileIcon('check', 14)} Dosya gönderildi${compressed ? ' (sıkıştırıldı)' : ''}`);
}
...
function renderFileMessage(msg) {
  ...
  let displayData = file.data;
  if (file.compressed && typeof GetticCompress !== 'undefined') {
    displayData = GetticCompress.compress(file.data); // decompress
  }
  
  switch (file.category) {
    case 'image': 
      return `<img src="${displayData}" alt="${escapeHtml(file.name)}" class="msg-image" loading="lazy" onclick="viewFile('${msg._id}')" style="max-width:300px;max-height:300px;border-radius:12px;cursor:pointer">`;
    case 'video': 
      return `<video src="${displayData}" controls style="max-width:300px;max-height:300px;border-radius:12px" preload="metadata"></video>`;
    case 'audio': 
      return `<audio src="${displayData}" controls style="width:250px"></audio>`;
    default: 
      return `<div class="file-attachment" onclick="downloadFile('${msg._id}')" style="background:var(--bg2);padding:10px 14px;border-radius:10px;cursor:pointer;display:flex;align-items:center;gap:8px">
        <span>${fileIcon('paperclip', 24)}</span>
        <div>
          <div style="font-weight:600;font-size:12px">${escapeHtml(file.name)} ${file.compressed ? '📦' : ''}</div>
...
function viewFile(msgId) {
  ...
  let displayData = msg.file.data;
  if (msg.file.compressed && typeof GetticCompress !== 'undefined') {
    displayData = GetticCompress.compress(msg.file.data);
  }
  ...
  content.innerHTML = `
    <div style="text-align:center">
      ${msg.file.category === 'image' ? `<img src="${displayData}" style="max-width:100%;max-height:70vh;border-radius:12px">` : 
        msg.file.category === 'video' ? `<video src="${displayData}" controls style="max-width:100%;max-height:70vh;border-radius:12px" autoplay></video>` : ''}
      <div style="margin-top:8px;display:flex;justify-content:center;gap:8px">
        <button class="mb sec" onclick="downloadFile('${msgId}')">${fileIcon('download', 16)} İndir</button>
        <button class="mb sec" onclick="copyFileLink('${msgId}')">${fileIcon('copy', 16)} Link Kopyala</button>
      </div>
    </div>`;
...
function downloadFile(msgId) {
  const msg = Store.messages.find(m => m._id === msgId);
  if (!msg?.file) return;
  let data = msg.file.data;
  if (msg.file.compressed && typeof GetticCompress !== 'undefined') {
    data = GetticCompress.compress(msg.file.data);
  }
  const a = document.createElement('a'); a.href = data; a.download = msg.file.name; a.click();
}
...
document.addEventListener('DOMContentLoaded', () => {
  if (!document.getElementById('fileUpload')) {
    ...
    input.type = 'file'; input.id = 'fileUpload'; 
...
console.log('✅ Files.js yüklendi (WASM Sıkıştırma + SVG İkonlar)');
