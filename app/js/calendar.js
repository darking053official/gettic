  ...
  content.innerHTML = `
    ...
    <div style="display:flex;flex-direction:column;gap:10px;margin-bottom:16px">
      <div style="display:flex;align-items:center;gap:8px;font-size:13px">
        <span>📆</span><span>${formatDate(event.date)} - ${event.time}</span>
        <span style="font-size:10px;color:var(--t3)">(${getTimeUntil(event.date, event.time)})</span>
      </div>
      ${event.description ? `<div style="display:flex;align-items:flex-start;gap:8px;font-size:13px"><span>📝</span><span>${event.description}</span></div>` : ''}
      <div style="display:flex;align-items:center;gap:8px;font-size:13px"><span>👤</span><span>${event.creatorName} tarafından oluşturuldu</span></div>
      <div style="display:flex;align-items:center;gap:8px;font-size:13px"><span>👥</span><span>${event.participants.length} katılımcı ${event.maxParticipants > 0 ? '/ ' + event.maxParticipants : ''}</span></div>
    </div>
    ${event.participants.length > 0 ? `
      <div class="settings-group"><div class="settings-group-title">Katılımcılar (${event.participants.length})</div>
        <div style="display:flex;flex-wrap:wrap;gap:6px">${event.participantNames.map(name => `<span style="font-size:11px;background:var(--bg2);padding:4px 10px;border-radius:12px">${name}</span>`).join('')}</div>
      </div>` : ''}
    <div style="display:flex;gap:8px;margin-top:16px">
      ${event.status !== 'past' && event.status !== 'cancelled' ? `<button class="mb ${isParticipant ? 'sec' : ''}" onclick="toggleEventRSVP('${event.id}');showEventDetail('${event.id}')">${isParticipant ? '👋 Ayrıl' : '✅ Katıl'}</button>` : ''}
      ${isCreator ? `<button class="mb sec" onclick="cancelEvent('${event.id}')">❌ İptal Et</button><button class="mb danger" onclick="deleteEvent('${event.id}');closeModal()">🗑️ Sil</button>` : ''}
    </div>
  `;
  openModal('event');
}
...
function showCalendar() {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    calendarHTML += `
      <div class="calendar-day ${isToday ? 'today' : ''}" onclick="showDayEvents('${dateStr}')">
        ...
        ${dayEvents.length > 0 ? `<div class="calendar-dots">${dayEvents.slice(0, 3).map(e => `<span class="calendar-dot" style="background:${e.color}" title="${e.title}"></span>`).join('')}${dayEvents.length > 3 ? `<span class="calendar-dot-more">+${dayEvents.length - 3}</span>` : ''}</div>` : ''}
      </div>`;
  }
  ...
  content.innerHTML = `
    ...
    <div class="calendar-grid">
      <div class="calendar-day header">Pzt</div><div class="calendar-day header">Sal</div><div class="calendar-day header">Çar</div><div class="calendar-day header">Per</div><div class="calendar-day header">Cum</div><div class="calendar-day header">Cmt</div><div class="calendar-day header">Paz</div>
      ${calendarHTML}
    </div>
    <div class="msep"></div>
    ...
    ${calendarState.events.filter(e => e.status === 'upcoming').sort((a,b) => new Date(a.date) - new Date(b.date)).slice(0, 5).map(e => `
      <div class="mitem" onclick="showEventDetail('${e.id}')">
        ...
        <div class="minfo"><div class="mname">${e.title}</div><div class="msub">${formatDate(e.date)} ${e.time} · ${e.participants.length} katılımcı</div></div>
...
function showDayEvents(dateStr) {
  ...
  content.innerHTML = `
    ...
    ${events.length === 0 ? '<p style="color:var(--t3);text-align:center;padding:20px">Bu tarihte etkinlik yok</p>' : events.map(e => `<div class="mitem" onclick="showEventDetail('${e.id}')"><div class="mav" style="background:${e.color};color:#fff">📅</div><div class="minfo"><div class="mname">${e.title}</div><div class="msub">${e.time} · ${e.participants.length} kişi</div></div></div>`).join('')}
...
function formatDate(dateStr) {
  try { const d = new Date(dateStr); return d.toLocaleDateString('tr-TR', { day: 'numeric', month: 'long', year: 'numeric' }); }
...
calStyle.textContent = `
  .calendar-grid { display: grid; grid-template-columns: repeat(7, 1fr); gap: 2px; margin-bottom: 12px; }
  .calendar-day { padding: 8px 4px; text-align: center; border-radius: 6px; cursor: pointer; font-size: 12px; min-height: 50px; transition: background .15s; }
  .calendar-day:hover { background: var(--bg2); }
  .calendar-day.header { font-weight: 700; font-size: 10px; color: var(--t3); cursor: default; min-height: auto; }
  .calendar-day.today { background: var(--acd); font-weight: 700; }
  .calendar-day.empty { cursor: default; }
  .calendar-day-num { display: block; margin-bottom: 4px; }
  .calendar-dots { display: flex; gap: 2px; justify-content: center; flex-wrap: wrap; }
  .calendar-dot { width: 5px; height: 5px; border-radius: 50%; display: inline-block; }
  .calendar-dot-more { font-size: 8px; color: var(--t3); }
`;
